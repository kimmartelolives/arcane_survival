...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    // 🛑 Don't start the step logic while the intro screen is still showing
    // Narration for step 1 is triggered directly in the Start button's onPointerDown
    if (showTutorialIntro) return;
    ...
    const interval = setInterval(() => {
      ...
      // ── STEP 3: Level-up card ───────────────────────────────────────────────
        ...
        }, 8000); // player reads the step message first
      }
  ...
  }, [screen, tutorialPhase, showTutorialIntro]);
...
const engineRef = useRef({
    ...
    // ⚡ PERF: reusable spatial-hash grid for enemy broad-phase collision (bullets, AoE).
    // Persisted on the engine object so the same Map is cleared+reused every frame
    // instead of being reallocated (avoids GC churn from creating a new Map 60x/sec).
    spatialGrid: new Map(),
    spatialGridCellSize: 96,
    // ⚡ PERF: free-list pool for particle objects. Dead particles are pushed
    // here instead of being garbage-collected, and spawnParticle() recycles
    // them instead of allocating new objects every spawn. This is the main
    // GC-pressure source at Wave 40+ (29 separate particle-spawn call sites,
    // some bursting 6-40 particles at once), so pooling avoids that churn.
    particlePool: [],
    bossIntro: {
        active: false,
        timer: 0,
...
// ⚡ PERF: PARTICLE OBJECT POOL
// Recycles dead particle objects instead of allocating a new one every spawn.
// All 8 fields are explicitly reset every time (including isGhost, defaulted
// to false) so a recycled object never leaks a stale property from whatever
// it was used for previously — this is the main source of GC pressure at
// Wave 40+ since particles spawn at 29 different call sites, some in bursts
// of up to 40 at once (e.g. shield-break effects).
  const spawnParticle = (eng, props) => {
    if (!eng.particlePool) eng.particlePool = [];
    let p;
    if (eng.particlePool.length > 0) {
      p = eng.particlePool.pop();
      p.x = props.x; p.y = props.y; p.vx = props.vx; p.vy = props.vy;
      p.color = props.color; p.life = props.life; p.ml = props.ml; p.r = props.r;
      p.isGhost = props.isGhost || false;
    } else {
      p = { x: props.x, y: props.y, vx: props.vx, vy: props.vy, color: props.color, life: props.life, ml: props.ml, r: props.r, isGhost: props.isGhost || false };
    }
  ...
  const resolveSigilCast = (currentEng, target, sigilType) => {
    ...
    if (sigilType === 'flareInferno') {
      ...
      for (const e of currentEng.enemies) {
        ...
        // ⚡ PERF: squared-distance — only relative ordering (nearest) matters here,
        // never the actual distance value, so sqrt is unnecessary.
      ...
      for (let i = 0; i < currentEng.enemies.length; i++) {
          ...
          // ⚡ PERF: squared-distance — only relative ordering (nearest) matters.
          const ddx2 = e.x - target.x, ddy2 = e.y - target.y;
          const dist = ddx2 * ddx2 + ddy2 * ddy2;
          if (dist < minDistance) {
              minDistance = dist;
              closestEnemy = e;
          }
 ...
 // 🔥 NEW: Catching Host Continuing
  ...
  useLayoutEffect(() => {
   ...
   if (screen === 'menu' || screen === 'lobby' || screen === 'tutorial') {
     ...
     // 🛑 PIPIGILAN NATIN ANG NORMAL WAVES PARA TUTORIAL ENEMIES LANG ANG LALABAS
      ...
      eng.gameStarted = true; 
      // 🛑 TUTORIAL: I-hold muna ang start hanggang ma-click ng player ang "Start Tutorial"
      if (screen === 'tutorial') {
        setHasStarted(false);
  ...
  useEffect(() => {
   ...
   // ════════════════════════════════════════════════════════════════
    ...
    // ════════════════════════════════════════════════════════════════
// ════════════════════════════════════════════════════════════════
    // THE VOID RUINS — v3: heavier, colder, more dangerous-looking.
    // Added: volumetric light shafts through the broken gate, debris
    // that floats instead of falling (because gravity is broken here),
    // a colossal cracked blade driven into the causeway, a snapped
    // chain still wrapped around the gate, a live arc of void-lightning
    // jumping between two rifts, and a final atmospheric grade pass so
    // the whole scene reads as one lit, breathing place instead of
    // flat shapes pasted on a canvas.
    // ════════════════════════════════════════════════════════════════
    ...
    // One light source for every cast shadow in the scene — discipline
    // here is what makes separate set-pieces read as one real place.
    const LIGHT = { dx: -0.42, dy: 0.30 };
    ...
    // A jagged static bolt of void-lightning — drawn once, frozen mid-arc,
    // like the air itself cracked between two points.
    const drawLightningArc = (g, x1, y1, x2, y2, seed, mainAlpha = 0.55) => {
      const segs = 7;
      const pts = [[x1, y1]];
      for (let i = 1; i < segs; i++) {
        ...
        const off = (seededRand(seed + i * 7.7) - 0.5) * 26 * Math.sin(t * Math.PI);
        pts.push([nx + Math.cos(perpA) * off, ny + Math.sin(perpA) * off]);
      }
      pts.push([x2, y2]);
      const stroke = (w, a, color) => {
        g.strokeStyle = color; g.lineWidth = w; g.globalAlpha = a;
        g.beginPath(); pts.forEach(([px2, py2], i) => i === 0 ? g.moveTo(px2, py2) : g.lineTo(px2, py2));
        g.stroke(); g.globalAlpha = 1;
      };
      g.save();
      g.shadowColor = 'rgba(170,210,255,0.9)'; g.shadowBlur = 16;
      stroke(3.2, mainAlpha * 0.35, 'rgba(170,210,255,1)');
      stroke(1.3, mainAlpha, 'rgba(225,238,255,1)');
      g.shadowBlur = 0;
      // a couple of small forking branches off the main bolt
      for (let b = 0; b < 2; b++) {
        const i = 2 + b * 2;
        if (!pts[i]) continue;
        const [bx, by] = pts[i];
        const ba = seededRand(seed + b * 13) * Math.PI * 2;
        const blen = 14 + seededRand(seed + b * 17) * 16;
        g.strokeStyle = `rgba(190,220,255,${mainAlpha * 0.5})`; g.lineWidth = 0.9;
        g.beginPath(); g.moveTo(bx, by); g.lineTo(bx + Math.cos(ba) * blen, by + Math.sin(ba) * blen); g.stroke();
      }
      g.restore();
    };
    ...
    // ── Impact crater — something heavy fell here once. Radial cracks
    // and a scorched ring give the floor a real story, not just decoration.
    fx.save();
    fx.translate(W*0.38, H*0.84);
    const craterGlow = fx.createRadialGradient(0,0,0,0,0,60);
    craterGlow.addColorStop(0, 'rgba(2,2,4,0.55)');
    craterGlow.addColorStop(0.5, 'rgba(2,2,4,0.22)');
    craterGlow.addColorStop(1, 'rgba(0,0,0,0)');
    fx.fillStyle = craterGlow; fx.fillRect(-60,-60,120,120);
    fx.strokeStyle = 'rgba(150,200,240,0.22)'; fx.lineWidth = 1.4;
    fx.shadowColor = 'rgba(150,200,240,0.4)'; fx.shadowBlur = 6;
    fx.beginPath(); fx.arc(0,0,34,0,Math.PI*2); fx.stroke();
    for (let i = 0; i < 8; i++) {
      const a = (i/8)*Math.PI*2 + seededRand(i*3.3)*0.3;
      const len = 30 + seededRand(i*5.5)*30;
      fx.beginPath(); fx.moveTo(Math.cos(a)*30, Math.sin(a)*30);
      fx.lineTo(Math.cos(a)*(30+len), Math.sin(a)*(30+len));
      fx.stroke();
    }
    fx.shadowBlur = 0;
    fx.restore();
    ...
    rifts.forEach(({ x, y, r, rot }, ri) => {
      ...
      // Reflective sheen on the stone right beside the rift — wet-looking
      // cold light catching the causeway surface, sells "real light source"
      sbx.save();
      sbx.translate(x, y + r*0.85);
      const sheen = sbx.createRadialGradient(0,0,0,0,0,r*1.3);
      sheen.addColorStop(0, 'rgba(160,205,255,0.10)');
      sheen.addColorStop(1, 'rgba(0,0,0,0)');
      sbx.scale(1.6, 0.4);
      sbx.fillStyle = sheen;
      sbx.beginPath(); sbx.arc(0,0,r*1.3,0,Math.PI*2); sbx.fill();
      sbx.restore();
    });

    // A live arc of void-lightning jumping between the two largest rifts —
    // frozen mid-strike, the most "something is still active here" beat.
    drawLightningArc(sbx, rifts[0].x, rifts[0].y, rifts[1].x, rifts[1].y, 4421, 0.4);

    // C. THE SHATTERED GATE — with volumetric light shafts pouring
    // through the broken opening, and a snapped chain still hanging
    // off the intact pillar (someone — or something — was chained here).
    sbx.save();
    ...
    // Volumetric light shafts — drawn first, underneath the stonework,
    // streaming down through the gap between the pillars
    sbx.save();
    const shaftCount = 4;
    for (let i = 0; i < shaftCount; i++) {
      const t = i / (shaftCount - 1);
      const baseX = -55 + t * 110;
      const spread = 30 + t * 14;
      sbx.beginPath();
      sbx.moveTo(baseX - 8, -190);
      sbx.lineTo(baseX + 8, -190);
      sbx.lineTo(baseX + spread, 260);
      sbx.lineTo(baseX - spread, 260);
      sbx.closePath();
      const sg = sbx.createLinearGradient(0, -190, 0, 260);
      sg.addColorStop(0, 'rgba(200,225,255,0.16)');
    ...
    const pillarDraw = (px3, h, brokenTop, tilt, withChain) => {
      sbx.save();
      sbx.translate(px3, 0);
      sbx.rotate(tilt);
      ...
      // Snapped chain, bolted high on the pillar, swinging loose at the
      // broken end — rusted links with a cold rim-light catching the edges
      if (withChain) {
        const anchorY = -h * 0.62;
        sbx.save();
        sbx.strokeStyle = '#1a1a20'; sbx.lineWidth = 5; sbx.lineCap = 'round';
        sbx.shadowColor = 'rgba(150,170,210,0.3)'; sbx.shadowBlur = 3;
        let lx = 18, ly = anchorY;
        sbx.beginPath(); sbx.moveTo(lx, ly);
        const linkN = 6;
        for (let li = 1; li <= linkN; li++) {
          const t = li/linkN;
          lx = 18 + Math.sin(t*4.2) * 14 + t*30;
          ly = anchorY + t*78 + t*t*22;
          sbx.lineTo(lx, ly);
        }
        sbx.stroke();
        // Highlight pass, thinner, cold light catching link edges
        sbx.strokeStyle = 'rgba(180,200,230,0.45)'; sbx.lineWidth = 1.6;
        sbx.shadowBlur = 0;
        sbx.stroke();
        // Broken end flares with a faint void glint
        sbx.shadowColor = 'rgba(160,205,255,0.7)'; sbx.shadowBlur = 8;
        sbx.fillStyle = 'rgba(190,215,250,0.6)';
        sbx.beginPath(); sbx.arc(lx, ly, 3, 0, Math.PI*2); sbx.fill();
        sbx.shadowBlur = 0;
        sbx.restore();
      }
      sbx.restore();
    };
    pillarDraw(-95, 215, false, -0.012, true);
    pillarDraw(95, 178, true, 0.05, false);
    ...
    // E2. FLOATING DEBRIS — chunks of broken stone hanging mid-air near
    // the rifts, because gravity doesn't quite hold here anymore. Each
    // still casts a real ground shadow directly below it, which is what
    // actually sells "floating" instead of "badly placed."
    const debris = [
      { x: W*0.30, y: H*0.66, hover: 34, size: 9, seed: 1 },
      { x: W*0.16, y: H*0.60, hover: 22, size: 6, seed: 2 },
      { x: W*0.74, y: H*0.26, hover: 28, size: 8, seed: 3 },
      { x: W*0.86, y: H*0.32, hover: 18, size: 5, seed: 4 },
      { x: W*0.18, y: H*0.18, hover: 24, size: 7, seed: 5 },
    ];
    debris.forEach(({ x, y, hover, size, seed }) => {
      // Ground contact shadow — smaller & softer the higher it floats
      sbx.save();
      sbx.translate(x, y + hover);
      sbx.fillStyle = `rgba(0,0,0,${0.22 - hover*0.003})`;
      sbx.beginPath(); sbx.ellipse(0, 0, size*1.1, size*0.4, 0, 0, Math.PI*2); sbx.fill();
      sbx.restore();
      // The floating rock itself
      sbx.save();
      sbx.translate(x, y - hover*0.3);
      sbx.rotate(seededRand(seed*6.6)*Math.PI*2);
      sbx.shadowColor = 'rgba(150,195,255,0.35)'; sbx.shadowBlur = 10;
      const rg = sbx.createLinearGradient(LIGHT.dx*size, -size, -LIGHT.dx*size, size);
      rg.addColorStop(0, '#5a5e6c'); rg.addColorStop(0.6, '#2c2d38'); rg.addColorStop(1, '#15161d');
      sbx.fillStyle = rg;
      sbx.strokeStyle = 'rgba(160,195,240,0.3)'; sbx.lineWidth = 0.8;
      sbx.beginPath();
      sbx.moveTo(-size,-size*0.3); sbx.lineTo(-size*0.2,-size); sbx.lineTo(size*0.8,-size*0.4);
      sbx.lineTo(size*0.7,size*0.6); sbx.lineTo(-size*0.6,size*0.5); sbx.closePath();
      sbx.fill(); sbx.stroke();
      sbx.shadowBlur = 0;
      sbx.restore();
    });
    ...
    // F2. THE COLOSSAL BLADE — a cracked, broken-tipped greatsword driven
    // into the causeway at the arena's heart. The clearest "maangas"
    // statement piece: someone won a fight here, or lost one badly.
    sbx.save();
    sbx.translate(W*0.50, H*0.74);
    sbx.rotate(0.06);
    const swordScale = Math.min(W,H) * 0.0016;
    sbx.scale(swordScale, swordScale);
    castShadow(sbx, 70, 220, 0.32);

    // Blade — long, half-buried, a chip out of the broken tip
    const bg2 = sbx.createLinearGradient(LIGHT.dx*30, -210, -LIGHT.dx*30, 30);
    bg2.addColorStop(0, '#eef3fb');
    bg2.addColorStop(0.4, '#9fb3cf');
    bg2.addColorStop(0.75, '#465062');
    bg2.addColorStop(1, '#1c2028');
    sbx.fillStyle = bg2;
    sbx.strokeStyle = 'rgba(180,200,230,0.5)'; sbx.lineWidth = 1.6;
    sbx.shadowColor = 'rgba(150,190,255,0.4)'; sbx.shadowBlur = 16;
    sbx.beginPath();
    sbx.moveTo(-14, 30);
    sbx.lineTo(-16, -150);
    sbx.lineTo(-9, -190);
    sbx.lineTo(0, -212); // tip
    sbx.lineTo(10, -184); // chipped notch back up — broken tip
    sbx.lineTo(5, -168);
    sbx.lineTo(16, -150);
    sbx.lineTo(14, 30);
    sbx.closePath();
    sbx.fill(); sbx.stroke();
    sbx.shadowBlur = 0;
    // Central fuller groove — a single bright line down the blade face
    sbx.strokeStyle = 'rgba(255,255,255,0.35)'; sbx.lineWidth = 1.2;
    sbx.beginPath(); sbx.moveTo(0,-205); sbx.lineTo(0,20); sbx.stroke();
    // Crack running diagonally across the lower blade
    sbx.strokeStyle = 'rgba(4,4,6,0.7)'; sbx.lineWidth = 1.4;
    sbx.beginPath(); sbx.moveTo(-10,-40); sbx.lineTo(9,10); sbx.stroke();
    sbx.shadowColor = 'rgba(150,200,250,0.6)'; sbx.shadowBlur = 5;
    sbx.strokeStyle = 'rgba(160,205,250,0.3)'; sbx.lineWidth = 0.6;
    sbx.stroke(); sbx.shadowBlur = 0;

    // Crossguard
    sbx.fillStyle = '#23242e';
    sbx.strokeStyle = 'rgba(170,190,225,0.4)'; sbx.lineWidth = 1.4;
    sbx.shadowColor = 'rgba(140,170,235,0.2)'; sbx.shadowBlur = 8;
    sbx.beginPath();
    sbx.moveTo(-42, 30); sbx.lineTo(42, 30); sbx.lineTo(36, 42); sbx.lineTo(-36, 42);
    sbx.closePath(); sbx.fill(); sbx.stroke();
    sbx.shadowBlur = 0;
    drawGlyphMark(sbx, 0, 36, 7, 0, 0.32, 'rgba(170,210,250,1)');

    // Hilt, half-sunk into the broken slab below it
    sbx.fillStyle = '#15161d';
    sbx.strokeStyle = 'rgba(150,165,200,0.3)'; sbx.lineWidth = 1.2;
    sbx.fillRect(-9, 42, 18, 34);
    sbx.strokeRect(-9, 42, 18, 34);
    for (let w = 0; w < 4; w++) {
      sbx.beginPath(); sbx.moveTo(-9, 48+w*8); sbx.lineTo(9, 48+w*8); sbx.stroke();
    }

    // Cracked slab beneath the blade, with void-light bleeding from the
    // wound it left when it landed
    sbx.save(); sbx.translate(0, 78);
    const wound = sbx.createRadialGradient(0,0,0,0,0,46);
    wound.addColorStop(0, 'rgba(150,195,255,0.22)');
    wound.addColorStop(1, 'rgba(0,0,0,0)');
    sbx.fillStyle = wound; sbx.beginPath(); sbx.ellipse(0,0,46,16,0,0,Math.PI*2); sbx.fill();
    sbx.strokeStyle = 'rgba(4,4,6,0.6)'; sbx.lineWidth = 1.2;
    for (let i = 0; i < 5; i++) {
      const a = (i/5)*Math.PI - Math.PI/2;
      sbx.beginPath(); sbx.moveTo(0,0); sbx.lineTo(Math.cos(a)*40, Math.sin(a)*12); sbx.stroke();
    }
    sbx.restore();
    sbx.restore();
    ...
    // H. SECONDARY VOID-VEINS from the rifts
    ...
    // I. FINAL ATMOSPHERIC GRADE — cinematic vignette + cool/warm split
    // tone. This single pass is what stops the scene from looking like
    // separate stickers and makes it look lit, photographed, real.
    sbx.save();
    // Top-of-frame haze, like distant depth fading into mist
    const depthFog = sbx.createLinearGradient(0, 0, 0, H);
    depthFog.addColorStop(0, 'rgba(150,170,210,0.10)');
    depthFog.addColorStop(0.3, 'rgba(150,170,210,0.0)');
    depthFog.addColorStop(1, 'rgba(0,0,0,0.06)');
    sbx.fillStyle = depthFog; sbx.fillRect(0, 0, W, H);
    // Vignette — darker corners pull focus to the gate / circle
    const vig = sbx.createRadialGradient(W*0.5, H*0.46, Math.min(W,H)*0.25, W*0.5, H*0.46, Math.max(W,H)*0.72);
    vig.addColorStop(0, 'rgba(0,0,0,0)');
    vig.addColorStop(1, 'rgba(0,0,0,0.40)');
    sbx.fillStyle = vig; sbx.fillRect(0, 0, W, H);
    sbx.restore();
      ...
      if (screenRef.current === 'tutorial' || screenRef.current === 'levelup') {
        ...
        if (eng.floatingTexts) {
          for (let i = eng.floatingTexts.length - 1; i >= 0; i--) {
            ...
            if (ft.life <= 0) {
              // ⚡ PERF: swap-pop instead of splice — O(1) removal. Safe here
              // since this only evicts *expired* entries (life<=0); the separate
              // 100-cap eviction above still uses shift() to preserve oldest-first
              // ordering for that rarer edge case.
              const lastFt = eng.floatingTexts.length - 1;
              if (i !== lastFt) eng.floatingTexts[i] = eng.floatingTexts[lastFt];
              eng.floatingTexts.pop();
            }
          }
        }
...
// 🔥 RED ZONES (AOE ATTACKS) LOGIC & FAST MATH COLLISION 🔥
                                ...
                                if (pTarget.divineShield && pTarget.divineShield.active) {
                                    ...
                                    damageTaken = 0;
                                        ...
                                        for(let k=0; k<40; k++) {
                                            ...
                                            spawnParticle(eng, { x: pTarget.x, y: pTarget.y, vx: Math.cos(pa)*ps, vy: Math.sin(pa)*ps, color: '#fef08a', life: 0.8, ml: 0.8, r: Math.random()*3+2 });
                                        }
                                    }
                                }
                                // -------------------------------------------------
          ...
          const applyPlayerDashAndMovement = (pObj, inX, inY, isP2) => {
            ...
            if (pObj.isDashing) {
                ...
                spawnParticle(eng, { x: pObj.x, y: pObj.y, vx: 0, vy: 0, color: isP2 ? '#f97316' : '#8b5cf6', life: 0.15, ml: 0.15, r: pObj.r, isGhost: true });
...
// 💥 UNIVERSAL PLAYER DAMAGE TAKEN TRACKER (BULLETPROOF RED TEXT)
        ...
        if (eng.slashes) {
          for (let slIdx = eng.slashes.length - 1; slIdx >= 0; slIdx--) {
            ...
            for (const enemy of eng.enemies) {
              // ⚡ PERF: squared-distance threshold check (no sqrt needed).
              const dxSl = enemy.x - sl.x, dySl = enemy.y - sl.y, rSumSl = enemy.r + 28;
              if ((dxSl * dxSl + dySl * dySl) < (rSumSl * rSumSl)) {
        ...
        if (eng.stars) {
          for (let sIdx = eng.stars.length - 1; sIdx >= 0; sIdx--) {
            ...
            if (star.progress >= 1) {
              ...
              for (const enemy of eng.enemies) {
                // ⚡ PERF: squared-distance threshold check (no sqrt needed).
                const dxSt = enemy.x - star.x, dySt = enemy.y - star.targetY;
                if ((dxSt * dxSt + dySt * dySt) <= (star.radius * star.radius)) {
          ...
          for (let cbIdx = eng.cubeBashes.length - 1; cbIdx >= 0; cbIdx--) {
            ...
            for (const enemy of eng.enemies) {
              ...
              if (enemy.hp > 0 && !cb.hitEnemies.has(enemy)) {
                // ⚡ PERF: squared-distance — pure threshold comparison.
                const dxCb2 = enemy.x - cb.x, dyCb2 = enemy.y - cb.y;
                let distSq = dxCb2 * dxCb2 + dyCb2 * dyCb2;
        ...
        if (eng.collapses) {
          ...
          for (let pIdx = eng.potions.length - 1; pIdx >= 0; pIdx--) {
            ...
            if (isCoopActive && eng.p2 && !eng.p2.dead) {
              // ⚡ PERF: squared-distance — only "which is closer" ordering matters.
              const ddxP1 = eng.p ? eng.p.x - pot.x : 0, ddyP1 = eng.p ? eng.p.y - pot.y : 0;
              const dP1 = eng.p ? (ddxP1 * ddxP1 + ddyP1 * ddyP1) : Infinity;
              const ddxP2 = eng.p2.x - pot.x, ddyP2 = eng.p2.y - pot.y;
              if ((ddxP2 * ddxP2 + ddyP2 * ddyP2) < dP1) {
                tx = eng.p2.x;
                ty = eng.p2.y; targetPlayer = eng.p2;
              }
            }
            if (!targetPlayer) continue;
            // ⚡ PERF: squared-distance threshold check.
            const ddxPd = tx - pot.x, ddyPd = ty - pot.y;
            const pdSq = ddxPd * ddxPd + ddyPd * ddyPd;
            if (pdSq < 12100) { // 110*110
              const ga = Math.atan2(ty - pot.y, tx - pot.x);
              pot.x += Math.cos(ga) * 260 * dt; pot.y += Math.sin(ga) * 260 * dt;
            }

            // ⚡ PERF: squared-distance threshold check.
            const ddxPk = targetPlayer.x - pot.x, ddyPk = targetPlayer.y - pot.y, rSumPk = targetPlayer.r + pot.r;
            if (!targetPlayer.dead && (ddxPk * ddxPk + ddyPk * ddyPk) < (rSumPk * rSumPk)) {
              ...
              if (pot.type === 'health') {
                ...
                for(let k=0; k<100; k++) {
                   const pa = Math.random()*Math.PI*2;
                   const ps = Math.random()*400+100;
                   spawnParticle(eng, { x: pot.x, y: pot.y, vx: Math.cos(pa)*ps, vy: Math.sin(pa)*ps, color: '#fef08a', life: 1.0, ml: 1.0, r: Math.random()*4+2 });
                }
              } else {
              ...
              if (pot.type !== 'nuke') {
                for(let k=0; k<8; k++) {
                  spawnParticle(eng, { x: pot.x, y: pot.y, vx: (Math.random()-0.5)*120, vy: (Math.random()-0.5)*120, color: '#f472b6', life: 0.25, ml: 0.25, r: 2 });
                }
              }
...
// 🎒 EQUIPMENT PICKUP LOGIC (NO MAGNET VERSION)
        if (eng.droppedItems) {
          for (let i = eng.droppedItems.length - 1; i >= 0; i--) {
            ...
            if (isCoopActive && eng.p2 && !eng.p2.dead) {
              // ⚡ PERF: squared-distance — only "which is closer" ordering matters.
              const ddxD1 = eng.p ? eng.p.x - drop.x : 0, ddyD1 = eng.p ? eng.p.y - drop.y : 0;
              const dP1 = eng.p ? (ddxD1 * ddxD1 + ddyD1 * ddyD1) : Infinity;
              const ddxD2 = eng.p2.x - drop.x, ddyD2 = eng.p2.y - drop.y;
              if ((ddxD2 * ddxD2 + ddyD2 * ddyD2) < dP1) {
                targetPlayer = eng.p2;
              }
            }
            ...
            // ⚡ PERF: squared-distance threshold check.
            const ddxDk = targetPlayer.x - drop.x, ddyDk = targetPlayer.y - drop.y, rSumDk = targetPlayer.r + 20;
            if (!targetPlayer.dead && (ddxDk * ddxDk + ddyDk * ddyDk) < (rSumDk * rSumDk)) {
       ...
       if (eng.gameStarted) {
          ...
          if (!isCoopActive || isHost) {
            // ⚡ PERF: SPATIAL HASH GRID — built once per frame from current enemy
            // positions. Bullet-vs-enemy collision below queries only the few cells
            // around each bullet instead of testing every enemy on the map. Identical
            // collision results to the brute-force version (same radius-sum test),
            // just far fewer comparisons once enemy counts climb past ~40-50
            // (e.g. boss waves where summons bypass the normal spawn cap).
            const grid = eng.spatialGrid;
            grid.clear();
            const cellSize = eng.spatialGridCellSize;
            // Collision-free cell key: offset both axes to always be positive (game
            // world + generous off-screen margin never approaches this range), then
            // pack into one number with a wide multiplier so the cx and cy "digits"
            // never overlap. (A plain XOR hash collides for negative coordinates —
            // e.g. cells above/left of the arena, which off-screen enemies/bullets
            // do occupy — which would have caused wrong bucket lookups.)
            const GRID_KEY_OFFSET = 100000;
            const GRID_KEY_MUL = 1000000;
            for (let gi = 0; gi < eng.enemies.length; gi++) {
              const ge = eng.enemies[gi];
              const gKey = (Math.floor(ge.x / cellSize) + GRID_KEY_OFFSET) * GRID_KEY_MUL + (Math.floor(ge.y / cellSize) + GRID_KEY_OFFSET);
              let bucket = grid.get(gKey);
              if (!bucket) { bucket = []; grid.set(gKey, bucket); }
              bucket.push(ge);
            }
            const queryNearbyEnemies = (qx, qy, qr, outArr) => {
              outArr.length = 0;
              const minCx = Math.floor((qx - qr) / cellSize);
              const maxCx = Math.floor((qx + qr) / cellSize);
              const minCy = Math.floor((qy - qr) / cellSize);
              const maxCy = Math.floor((qy + qr) / cellSize);
              for (let cx = minCx; cx <= maxCx; cx++) {
                for (let cy = minCy; cy <= maxCy; cy++) {
                  const key = (cx + GRID_KEY_OFFSET) * GRID_KEY_MUL + (cy + GRID_KEY_OFFSET);
                  const bucket = grid.get(key);
                  if (bucket) {
                    for (let bi = 0; bi < bucket.length; bi++) outArr.push(bucket[bi]);
                  }
                }
              }
              return outArr;
            ...
            for (let i = eng.bullets.length - 1; i >= 0; i--) {
              ...
              if (b.life <= 0 || b.x < -20 || b.x > W + 20 || b.y < -20 || b.y > H + 20) { 
                // ⚡ PERF: swap-pop instead of splice — O(1) removal, no visual
                // difference since bullets are drawn independently (no order/z dependency).
                const lastB = eng.bullets.length - 1;
                if (i !== lastB) eng.bullets[i] = eng.bullets[lastB];
                eng.bullets.pop();
                continue; 
              }
              let hit = false;
              ...
              if (b.isEnemy) {
                for (const pTarget of [eng.p, isCoopActive ? eng.p2 : null]) {
                  // ⚡ PERF: squared-distance threshold check (no sqrt needed).
                  const dxBP = pTarget ? b.x - pTarget.x : 0, dyBP = pTarget ? b.y - pTarget.y : 0;
                  const rSumBP = pTarget ? b.r + pTarget.r : 0;
                  if (pTarget && !pTarget.dead && pTarget.inv <= 0 && (dxBP * dxBP + dyBP * dyBP) < (rSumBP * rSumBP)) {
                    ...
                    // ⚡ PERF: swap-pop instead of splice — O(1) removal.
                    hit = true;
                    { const lastB = eng.bullets.length - 1; if (i !== lastB) eng.bullets[i] = eng.bullets[lastB]; eng.bullets.pop(); }
                    break;
                  }
                }
                continue;
                ...
                // ⚡ PERF: query only enemies in nearby grid cells instead of the whole
                // eng.enemies array. Same radius-sum collision test as before — this
                // only narrows *which* enemies get tested, not how the test works.
                // Padding (70) safely covers the largest enemy radius in the game (boss r=60).
                const nearbyEnemies = queryNearbyEnemies(b.x, b.y, b.r + 70, _nearbyScratch);
                for (let j = nearbyEnemies.length - 1; j >= 0; j--) {
                const e = nearbyEnemies[j];
                // ⚡ PERF: squared-distance check (no Math.sqrt) — mathematically identical
                // result to Math.hypot(...) < (b.r+e.r) since both sides are non-negative.
                const dxBE = b.x - e.x, dyBE = b.y - e.y;
                const rSumBE = b.r + e.r;
                if ((dxBE * dxBE + dyBE * dyBE) < (rSumBE * rSumBE)) {
                  ...
                  for(let k=0; k<5; k++) {
                    ...
                    spawnParticle(eng, { x: b.x, y: b.y, vx: Math.cos(pa)*ps, vy: Math.sin(pa)*ps, color: e.color, life: 0.3, ml: 0.3, r: 2 });
                  }
                  // ⚡ PERF: swap-pop instead of splice — O(1) removal.
                  { const lastB = eng.bullets.length - 1; if (i !== lastB) eng.bullets[i] = eng.bullets[lastB]; eng.bullets.pop(); }
                  hit = true;
        ...
        if (e.stigmaTime > 0) {
          ...
          if (Math.random() < 0.15) {
            spawnParticle(eng, { x: e.x + (Math.random() - 0.5) * 10, y: e.y + (Math.random() - 0.5) * 10, vx: 0, vy: -15, color: '#f43f5e', life: 0.25, ml: 0.25, r: 1.5 });
          }
          if (e.hp <= 0) e.deadTrigger = true;
        }
              ...
              if (e.arcaneBurnTime > 0) {
                ...
                e.hp -= 45 * dt; 
                if (Math.random() < 0.22) {
                  spawnParticle(eng, { x: e.x + (Math.random()-0.5)*16, y: e.y + (Math.random()-0.5)*16, vx: (Math.random()-0.5)*15, vy: -25, color: '#d946ef', life: 0.3, ml: 0.3, r: 2 });
                }
                if (e.hp <= 0) e.deadTrigger = true;
              }
          ...
          if (eng.wave === 100 && (e.type === 'abyss_awakened' || e.type === 'primordial')) {
              ...
              // Play Cinematic with DELAY
                ...
                // ⚡ PERF: swap-pop instead of splice — O(1) removal vs O(n) array
                // shift. Safe: nothing depth-sorts eng.enemies by array position,
                // and hit-tracking Sets (cb.hitEnemies, sl.hits) key by object
                // identity, not array index, so reordering doesn't affect them.
                { const lastE = eng.enemies.length - 1; if (j !== lastE) eng.enemies[j] = eng.enemies[lastE]; eng.enemies.pop(); }
                continue;
              }
              ...
              if (e.stunnedTime > 0) {
                ...
                if (isCoopActive && eng.p2 && !eng.p2.dead) {
                  // ⚡ PERF: squared-distance — only "which player is closer" ordering matters.
                  const ddxD1 = e.x - eng.p?.x, ddyD1 = e.y - eng.p?.y;
                  const d1 = (!eng.p || eng.p.dead) ? Infinity : (ddxD1 * ddxD1 + ddyD1 * ddyD1);
                  const ddxD2 = e.x - eng.p2.x, ddyD2 = e.y - eng.p2.y;
                  const d2 = ddxD2 * ddxD2 + ddyD2 * ddyD2;
                  if (d2 < d1) tx = eng.p2;
                }
                if (tx) {
              ...
              // ⚡ PERF: squared-distance threshold check (no sqrt needed).
              { const dxEp1 = e.x - eng.p?.x, dyEp1 = e.y - eng.p?.y, rSumEp1 = eng.p ? (e.r + eng.p.r) : 0;
              if (eng.p && !eng.p.dead && eng.p.inv <= 0 && (dxEp1 * dxEp1 + dyEp1 * dyEp1) < (rSumEp1 * rSumEp1)) {
                ...
                if (eng.p.divineShield && eng.p.divineShield.active) {
                    if (eng.p.divineShield.hp >= damageTaken) {
                        ...
                        for(let k=0; k<40; k++) {
                            const pa = Math.random() * Math.PI * 2;
                            const ps = Math.random() * 350 + 100;
                            spawnParticle(eng, { x: eng.p.x, y: eng.p.y, vx: Math.cos(pa)*ps, vy: Math.sin(pa)*ps, color: '#fef08a', life: 0.8, ml: 0.8, r: Math.random()*3+2 });
                        }
                    }
                }
                // -------------------------------------------------
              ...
              // ⚡ PERF: squared-distance threshold check (no sqrt needed).
              { const dxEp2 = e.x - (eng.p2?.x ?? 0), dyEp2 = e.y - (eng.p2?.y ?? 0), rSumEp2 = eng.p2 ? (e.r + eng.p2.r) : 0;
              if (isCoopActive && eng.p2 && !eng.p2.dead && eng.p2.inv <= 0 && (dxEp2 * dxEp2 + dyEp2 * dyEp2) < (rSumEp2 * rSumEp2)) {
                 ...
                 // --- 👼 HOLY SERAPH: DIVINE ABSORPTION SHIELD (P2) ---
                    if (eng.p2.divineShield && eng.p2.divineShield.active) {
                        ...
                        for(let k=0; k<40; k++) {
                            const pa = Math.random() * Math.PI * 2;
                            const ps = Math.random() * 350 + 100;
                            spawnParticle(eng, { x: eng.p2.x, y: eng.p2.y, vx: Math.cos(pa)*ps, vy: Math.sin(pa)*ps, color: '#fef08a', life: 0.8, ml: 0.8, r: Math.random()*3+2 });
                        }
                    }
                }
                // -------------------------------------------------
            ...
            for (let i = eng.gems.length - 1; i >= 0; i--) {
              ...
              if (g.life <= 0) {
                // ⚡ PERF: swap-pop instead of splice — O(1) removal, no visual
                // difference (gems drawn independently, no order dependency).
                const lastG = eng.gems.length - 1;
                if (i !== lastG) eng.gems[i] = eng.gems[lastG];
                eng.gems.pop();
                continue;
              }
              let tx = eng.p ? eng.p.x : W/2, ty = eng.p ? eng.p.y : H/2; let targetPlayer = eng.p;
              if (isCoopActive && eng.p2 && !eng.p2.dead) {
                // ⚡ PERF: squared-distance — only "which is closer" ordering matters.
                const ddxG1 = eng.p ? eng.p.x - g.x : 0, ddyG1 = eng.p ? eng.p.y - g.y : 0;
                const dP1 = eng.p ? (ddxG1 * ddxG1 + ddyG1 * ddyG1) : Infinity;
                const ddxG2 = eng.p2.x - g.x, ddyG2 = eng.p2.y - g.y;
                if ((ddxG2 * ddxG2 + ddyG2 * ddyG2) < dP1) {
                  tx = eng.p2.x;
                  ty = eng.p2.y; targetPlayer = eng.p2;
                }
              }
              ...
              if (!targetPlayer) continue;
              // ⚡ PERF: squared-distance threshold check.
              const ddxGd = tx - g.x, ddyGd = ty - g.y;
              const gdSq = ddxGd * ddxGd + ddyGd * ddyGd;
              ...
              // ⚡ PERF: squared-distance threshold check.
              const ddxGk = targetPlayer.x - g.x, ddyGk = targetPlayer.y - g.y, rSumGk = targetPlayer.r + g.r;
              if (!targetPlayer.dead && (ddxGk * ddxGk + ddyGk * ddyGk) < (rSumGk * rSumGk)) {
              ...
              if (isCoopActive && targetPlayer === eng.p2) {
                ...
                // ⚡ PERF: swap-pop instead of splice — O(1) removal.
                { const lastG = eng.gems.length - 1; if (i !== lastG) eng.gems[i] = eng.gems[lastG]; eng.gems.pop(); }
              }
            }
          }
...
// =========================================================
    ...
    // 🔥 ANTI-LAG SYSTEM: I-cap ang particles sa 300 para hindi mag-crash ang browser sa late game
      ...
      // discarding the object. Splice shifts every subsequent element down by
      // one (O(n) per removal); discarding objects forces constant GC churn
      // at 29 particle-spawn call sites. Swap-pop is O(1), and recycling into
      // particlePool avoids the allocation entirely on the next spawn.
      // IMPORTANT: `p` here is the ORIGINAL object at index i, captured
      // before the slot is overwritten by the swap — pushing this exact
      // reference to the pool (rather than re-reading eng.particles[i] after
      // the swap) is what avoids a subtle aliasing bug: if we instead pooled
      // whatever ended up at index i post-swap, that could be the SAME
      // object that the swap just copied in from the last slot, and later
      // mutating it as a "recycled" particle would corrupt the live one
      // still sitting in the array.
      for (let i = eng.particles.length - 1; i >= 0; i--) {
        const p = eng.particles[i];
        p.x += p.vx * dt; p.y += p.vy * dt; p.vx *= 0.93; p.vy *= 0.93; p.life -= dt;
        if (p.life <= 0) {
          const last = eng.particles.length - 1;
          if (i !== last) eng.particles[i] = eng.particles[last];
          eng.particles.pop();
          eng.particlePool.push(p);
        }
      }
  ...
  if (pObj.divineShield) {
      ...
      if (pObj.divineShield.duration <= 0) {
          ...
          for(let k=0; k<20; k++) {
              ...
              spawnParticle(eng, { x: pObj.x, y: pObj.y, vx: Math.cos(pa)*ps, vy: Math.sin(pa)*ps, color: 'rgba(254, 240, 138, 0.5)', life: 0.5, ml: 0.5, r: Math.random()*2+1 });
          }
  ...
  pObj.familiars.forEach((f, index) => {
    ...
    if (f.cd <= 0) {
      if (f.id === 'wisp') {
        ...
        for(let k=0; k<6; k++) spawnParticle(eng, { x: pObj.x + (Math.random()-0.5)*30, y: pObj.y + (Math.random()-0.5)*30, vx: 0, vy: -40 - Math.random()*20, color: '#86efac', life: 0.8, ml: 0.8, r: 2.5 });
      }
      else if (f.id === 'frost') {
      ...
      // 🦇 UMBRAL BAT (Shadow Blades)
        ...
        if (hasValidShield) {
            ...
            for(let k=0; k<25; k++) {
                const pa = Math.random() * Math.PI * 2;
                const ps = Math.random() * 200 + 50;
                spawnParticle(eng, { x: pObj.x, y: pObj.y, vx: Math.cos(pa)*ps, vy: Math.sin(pa)*ps, color: '#fef08a', life: 1.0, ml: 1.0, r: Math.random()*4+2 });
            }
        }
      }
      else if (f.id === 'wind') {
...
if (isHost || !isCoopActive) {
  ...
  if (isCoopActive) tickFamiliars(eng.p2, true);
    ...
    const renderLoop = () => {
      // ⚡ PERF: VFX FRAME-BUDGET GATE
      // renderLoop runs at the display's native refresh rate (60/90/120Hz),
      // separate from the fixed-60Hz simulation tick in worker.onmessage.
      // Several boss-specific particle systems (demon eyes, smoke, sakura
      // petals, etc.) spawn and age their particles directly inside this
      // render path, so on a 120Hz phone they were spawning/aging roughly
      // twice as fast as intended. shouldUpdateVfx approximates a steady
      // ~60Hz cadence for just that spawn/age logic, regardless of the
      // device's actual refresh rate — the particles still get drawn every
      // single render frame either way, so motion stays smooth.
      const _nowVfx = performance.now();
      if (!eng._lastVfxTick) eng._lastVfxTick = _nowVfx;
...
if (skin === 'shadow') {
    ...
    ctx.restore();
      ...
      for (const e of eng.enemies) {
        ...
        // (ctx.drawImage) further down and never read fillStyle/shadowColor/
        // shadowBlur — their glow is already baked into the cached sprite.
        // Skipping these three state writes for them avoids wasted canvas
        // state changes on the bulk of every wave's enemy count, with zero
        // visual difference (the skipped branches genuinely never use them).
        if (e.type !== 'normal' && e.type !== 'fast' && e.type !== 'tank') {
...
// 🟢 SKIN AESTHETICS HELPER (Nagpapalit ng kulay ng damit, sumbrero at glow)
      ...
      const drawAdvancedSkinAura = (x, y, radius, skinId) => {
          ...
          if (skinId === 'shadow') {
              ...
              // refresh rate, so 120Hz phones spawned/aged eyes ~2x too fast).
              if (shouldUpdateVfx) {
                ...
                for (let i = eng.demonEyes.length - 1; i >= 0; i--) {
                  const eye = eng.demonEyes[i];
                  eye.life -= 0.02; eye.blink += 0.2;
                  if (eye.life <= 0) eng.demonEyes.splice(i, 1);
                }
              }
              // Drawing still runs every render frame so motion stays smooth.
              eng.demonEyes.forEach((eye) => {
          ...
          // 🔥 PYROMANCER (DEMON OF THE ASHEN FLARE)
              ...
              // ⚡ PERF: spawn + physics gated to ~60Hz (was running at display
              // refresh rate). Drawing still runs every render frame so motion
              // stays smooth; only the underlying physics tick rate changed.
              if (shouldUpdateVfx) {
                ...
                for (let i = eng.pyroSmoke.length - 1; i >= 0; i--) {
                  const s = eng.pyroSmoke[i];
                  s.oy -= s.vy; s.ox += Math.sin(time*0.005 + i); s.life -= 0.015; s.size += 0.2;
                  if (s.life <= 0) eng.pyroSmoke.splice(i, 1);
                }
              }
              
              eng.pyroSmoke.forEach((s) => {
                  if (s.isFire) {
              ...
              if (eng.p.divineShield.duration !== undefined) {
                  ...
                  if (eng.p.divineShield.duration <= 0) {
                      ...
                      for(let k=0; k<20; k++) {
                          const pa = Math.random() * Math.PI * 2;
                          const ps = Math.random() * 150 + 50;
                          spawnParticle(eng, { x: p1X, y: p1Y, vx: Math.cos(pa)*ps, vy: Math.sin(pa)*ps, color: 'rgba(254, 240, 138, 0.5)', life: 0.5, ml: 0.5, r: Math.random()*2+1 });
                      }
                  }
              }
              // =======================================================
              ...
              if (eng.p2.divineShield.duration !== undefined) {
                  ...
                  if (eng.p2.divineShield.duration <= 0) {
                      ...
                      for(let k=0; k<20; k++) {
                          const pa = Math.random() * Math.PI * 2;
                          const ps = Math.random() * 150 + 50;
                          spawnParticle(eng, { x: p2X, y: p2Y, vx: Math.cos(pa)*ps, vy: Math.sin(pa)*ps, color: 'rgba(254, 240, 138, 0.5)', life: 0.5, ml: 0.5, r: Math.random()*2+1 });
                      }
                  }
              }
...
{showTutorialUpgrade && (
    ...
    {screen === 'tutorial' && !showTutorialComplete && (
      ...
      {screen === 'tutorial' && showTutorialIntro && (
        <>
          <style>{`
            /* ── Shared keyframes (duplicated here so they exist before complete screen mounts) ── */
            ...
            @keyframes tut-intro-step-badge-pulse {
            ...
            .tut-intro-step-badge {
              ...
              border-radius: 6px;
              padding: 5px 10px;
              animation: tut-intro-step-badge-pulse 4s ease-in-out infinite;
            }
            .tut-intro-step-badge:nth-child(2){animation-delay:.7s}
            .tut-intro-step-badge:nth-child(3){animation-delay:1.4s}
            .tut-intro-step-badge:nth-child(4){animation-delay:2.1s}
            .tut-intro-step-badge:nth-child(5){animation-delay:2.8s}
            .tut-intro-step-num {
              font-family:'Cinzel','Georgia',serif;
              font-size:8px; font-weight:700; letter-spacing:1.5px;
              color:#AFA9EC; min-width:44px; text-align:center; text-transform:uppercase;
            }
            .tut-intro-step-text {
              font-family:'Cinzel','Georgia',serif;
              font-size:9px; color:#8b9cc8; line-height:1.5; letter-spacing:.3px;
            }
            .tut-btn-start {
              background: rgba(83,74,183,0.18);
          ...
          <div className="tut-intro-overlay">
            <div className="tut-complete-modal">
              ...
              {/* Layered spinning sigil — exact same as complete screen */}
              ...
              {/* Title */}
              <h2 className="tut-intro-title">Wizard Training Grounds</h2>
              ...
              {/* Subtitle */}
              <p className="tut-intro-sub">
                Master the fundamentals before entering Arcane Realm.
              </p>

              {/* Steps overview */}
              <div className="tut-intro-steps-list">
                <div className="tut-intro-step-badge">
                  <span className="tut-intro-step-num">Step 1</span>
                  <span className="tut-intro-step-text">Move through the wizard training grounds</span>
                </div>
                <div className="tut-intro-step-badge">
                  <span className="tut-intro-step-num">Step 2</span>
                  <span className="tut-intro-step-text">Dodge arcane creatures and survive for 8 seconds</span>
                </div>
                <div className="tut-intro-step-badge">
                  <span className="tut-intro-step-num">Step 3</span>
                  <span className="tut-intro-step-text">Level up and choose your first arcane upgrade</span>
                </div>
                <div className="tut-intro-step-badge">
                  <span className="tut-intro-step-num">Step 4</span>
                  <span className="tut-intro-step-text">Use your Dash and defeat an enemy</span>
                </div>
                <div className="tut-intro-step-badge">
                  <span className="tut-intro-step-num">Step 5</span>
                  <span className="tut-intro-step-text">Unleash Flare Inferno to slay the Boss</span>
                </div>
              </div>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
                <button
                  className="tut-btn tut-btn-start"
                  onPointerDown={(e) => {
                    e.stopPropagation();
                    if (window.ArcaneSoundManager) window.ArcaneSoundManager.unlockAll();
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
                <button
                  ...
                  onPointerDown={(e) => {
                    ...
                    // 🔥 ROOT CAUSE FIX:
                    // Ang screen ay 'tutorial' na, kaya setScreen('tutorial') ay
                    // walang epekto — hindi mag-fi-fire ang useLayoutEffect kung
                    // hindi nagbabago ang value. Kaya setShowTutorialIntro(true)
                    // at playNarration('/step1.mp3') ay hindi nata-trigger.
                    // Solusyon: i-reset nang direkta dito lahat ng kailangan.
                    ...
                    setIsPreloading(true);
                    setShowTutorialIntro(true);
                    // step1.mp3 ay ita-trigger ng "Start the Training" button (tulad ng dati)
                  }}
                >
                  ↺ &nbsp; Replay Tutorial
                </button>
